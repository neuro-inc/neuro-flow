...
def test_seq_batch(assets: pathlib.Path, run_cli: RunCLI):
...
def test_batch_with_local(assets: pathlib.Path, run_cli: RunCLI):
    captured = run_cli(
        [
            "bake",
            "print-local",
...
def test_batch_action(assets: pathlib.Path, run_cli: RunCLI):
